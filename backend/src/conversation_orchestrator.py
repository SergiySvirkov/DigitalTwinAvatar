"""
Conversation Orchestrator - Async pipeline coordinating LLM, memory, documents, and response generation
Manages the flow of conversation processing with low latency and document RAG support
"""
...
class ConversationOrchestrator:
    """Coordinates all conversation processing components including document RAG"""
    ...
    async def process_message(
        ...
        """Process a user message through the full pipeline including document RAG"""
        ...
        context_tasks = [
            ...
            self._retrieve_document_context(message)  # NEW: Document RAG
        ]

        system_prompt, conversation_history, relevant_memories, document_context = await asyncio.gather(*context_tasks)
        ...
        messages = self._build_messages(
            ...
            document_context=document_context,  # NEW: Include document context
    ...
    async def _retrieve_document_context(self, query: str) -> str:
        """
        Retrieve relevant document context using FAISS-based RAG.
        ...
        try:
            ...
            results = await self.document_rag.search(query, top_k=3)
            
            if not results:
                return ""
            
            # Format context for LLM with source attribution
            return self.document_rag.format_context_for_llm(results)
    ...
    def _build_messages(
        ...
        document_context: str,  # NEW: Document context parameter
        current_message: str,
        vision_context: Optional[str] = None
    ) -> List[Dict[str, str]]:
        """Build message list for LLM with document citations support"""
        messages = []

        # Build enhanced system prompt with citation instructions
        full_system = system_prompt

        # Add citation instructions if document RAG is available
        if self.document_rag:
            full_system += """

IMPORTANT INSTRUCTIONS FOR DOCUMENT CITATIONS:
- When referencing uploaded documents, cite sources naturally in your response
- Use phrases like "According to the document...", "Based on the uploaded file...", "The document mentions...", or "As stated in [title]..."
- Do NOT use technical IDs or chunk references like "[doc_123]" or "[chunk_456]"
- If the user's question relates to uploaded documents, incorporate relevant information naturally
- If no relevant documents are found, respond based on your general knowledge without mentioning documents
- Always maintain a conversational, natural tone when citing sources
"""
